...
function kpi(
  data: { kpis: { id: string; value: string; previous: string | null; deltaPct: number | null }[] },
  id: string,
) {
...
describe.skipIf(!HAS_TEST_DB)('لوحة التحكم — Increment 3', () => {
  ...
  it('طلب مؤكد 1000 + دفعة 400: طلب/تحصيل/دَين', async () => {
  ...
  it('يحسب المسودة ضمن فترة تأكيدها لا فترة إنشائها', async () => {
    const cid = await createTestCustomer(t, 'زبون');
    const draft = await asUser(t, () => orders.create(orderPayload(cid, { status: 'DRAFT' })));
    await inTenant(t.tenantId, (tx) =>
      tx.order.update({ where: { id: draft.id }, data: { issuedAt: new Date('2020-01-01') } }),
    );

    await asUser(t, () => orders.confirm(draft.id, { version: draft.version }));
    const data = await asUser(t, () => dashboard.getDashboard(Q()));

    expect(kpi(data, 'orders')?.value).toBe('1');
  });

  it('لا ترفع دفعة دين قديم نسبة تحصيل طلبات الفترة الحالية', async () => {
    const cid = await createTestCustomer(t, 'زبون', { creditLimit: '5000' });
    const oldOrder = await asUser(t, () => orders.create(orderPayload(cid)));
    await inTenant(t.tenantId, (tx) =>
      tx.order.update({
        where: { id: oldOrder.id },
        data: { confirmedAt: new Date('2020-01-01'), dueAt: new Date('2020-01-02') },
      }),
    );
    await asUser(t, () => orders.create(orderPayload(cid)));
    await pay(cid, '400');
  ...
  it('طلب متأخر مدفوع جزئيًا: المتأخرات = الإجمالي − المدفوع', async () => {
    ...
    await inTenant(t.tenantId, (tx) =>
      tx.$executeRawUnsafe(
        `UPDATE orders SET due_at = now() - interval '10 days' WHERE id=$1::uuid`,
        o.id,
      ),
    );
  ...
  it('قيمة عشرية كبيرة تُحفظ بدقّة (بلا فاصلة عائمة)', async () => {
    ...
    const data = await asUser(t, () => dashboard.getDashboard(Q()));
    expect(data.topCustomers[0]?.amount).toBe('999999.99');
  });
  ...
  it('فترة مخصّصة في الماضي تستبعد طلب اليوم', async () => {
    ...
    expect(kpi(past, 'orders')?.value).toBe('0');
  });

  it('مقارنة الفترة السابقة: طلبات هذا الشهر مقابل صفر الشهر الماضي', async () => {
    const cid = await createTestCustomer(t, 'زبون', { creditLimit: '5000' });
    await asUser(t, () => orders.create(orderPayload(cid)));

    const data = await asUser(t, () => dashboard.getDashboard(Q()));
    const orderCount = kpi(data, 'orders');
    expect(orderCount?.value).toBe('1');
    expect(orderCount?.previous).toBe('0');
    expect(orderCount?.deltaPct).toBe(100);
  });
