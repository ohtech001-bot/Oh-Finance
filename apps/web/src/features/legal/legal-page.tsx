    ...
    <main className="min-h-dvh bg-[#090a08] text-white">
      <header className="border-b border-amber-400/20 bg-black/70">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <Link
            to="/login"
            className="flex items-center gap-2 text-sm text-white/75 transition-colors hover:text-amber-300"
          >
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="mb-8 flex items-center gap-4">
          <div className="rounded-icon flex size-12 shrink-0 items-center justify-center border border-amber-400/30 bg-amber-400/10">
          ...
          {SECTION_KEYS[kind].map((section) => (
            <section key={section} className="py-6">
              <h2 className="text-base font-semibold text-amber-200">
                {t(`legal.${kind}.${section}Title`)}
              </h2>
              <p className="mt-2 text-sm leading-7 text-white/70">
                {t(`legal.${kind}.${section}Body`)}
              </p>
            </section>
          ))}
        </div>
